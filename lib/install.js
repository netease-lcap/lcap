...
function getLcapLibName({ framework, type, version }) {
  ...
  if (framework === 'vue3') {
    return type === 'h5' ? '@lcap/vant' : '@lcap/element-plus';
  }
...
async function addVueCompositionAPI(rootPath) {
  ...
  try {
    await fs.outputFile(pkgPath, JSON.stringify(pkg, null, 2));
  } catch (e) {
    console.log(e);
  }
}
...
async function installLcapUI({ platform, framework, version, type, pkgName }, rootPath = process.cwd()) {
  ...
  if (['@lcap/element-ui', '@lcap/pc-ui', '@lcap/mobile-ui'].includes(pkgName)) {
...
async function promptInstallUI(framework) {
  const answer = await inquirer.prompt([
    ...
    },
    {
      ...
      choices: [
        { value: '@lcap/pc-ui', name: 'CloudUI（官方默认）' },

...
const callZipUsage = async () => {
    const dirList = ['usage.json', 'manifest'];
    const fileList = glob.sync(`dist-theme/**`, { root }).filter((item) => item.indexOf('.') !== -1);
...
const callZipExtension = async () => {
    const dirList = ['nasl.extension.json', 'manifest'];
    const fileList = glob.sync(`dist-theme/**`, { root }).filter((item) => item.indexOf('.') !== -1).concat(['nasl.extension.d.ts']);
    const zipList = dirList.concat(fileList);
    const pkg = require(path.resolve(root, 'package.json'));
    const manifestData = {
        'Plugin-Version': '1.0.0',
...
const callZip = (extension) => {
    if (!extension) {
        return callZipUsage();
    }

    return callZipExtension();
};

    ...
    Object.keys(structureList).forEach((item) => {
        ...
        const resultStructureList = Object.keys(structureData).map((it) => {
            ...
            const obj = {
                ...
                ...formatTypeAnnotation(list[0]),
            };
            return obj;
        });
        obj.name = item;
        obj.properties = resultStructureList;
        resultList.push(obj);
    });
...
/* 获取数据类型 */
function formatTypeAnnotation(type, obj) {
    // const result = /(\[\])/.exec(type);
    const typeAnnotation = obj || {
    ...
    //     formatTypeAnnotation(result[2], typeAnnotation.typeArguments[0]);
    // } else {
    const baseTypeList = ['Inerger', 'Boolean', 'Decimal', 'String', 'Date', 'DateTime', 'Time'];

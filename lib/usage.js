...
function getNewUsage(dir) {
    const fileList = glob.sync(`*(components|logics|annotations)/**/api.yaml`, {
        root: dir,
    });
    if (!fileList.length) {
        return '';
    ...
    const descList = fileList.map((item) => {
        ...
        if (~item.indexOf('logics') && !obj[0].params || item.includes('annotations')) {
    ...
    const resConfiguration = getConfiguration();
    const res = assignUsage(descList.flat(1), pkg);
    return Object.assign(res, resStructure, resConfiguration);
}
/* 获取数据结构 */
...
function getConfiguration() {
    let properties = [];
    try {
        const strPath = path.resolve(process.cwd(), 'configuration/index.json');
        const isFile = fs.existsSync(strPath);
        if (!isFile) {
            return;
        }
        const configurationList = require(strPath);
        properties = configurationList.map((item) => { item.concept = 'ConfigProperty'; return item; }) || [];
        return ({
            configuration: {
        ...
        console.error('configuration格式不对,请检查');
    }
    return {};
}
...
function assignUsage(list, pkg) {
    ...
    list.forEach((item) => {
        ...
        } else if (item.belong === 'annotation') {
            const logic = {
                concept: 'Logic',
                name: item.name,
                description: item.description,
    ...
    if (h5Components.length || h5Logics.length) {
        ...
        summary.frontends.push({
            type: 'h5',
            viewComponents: h5Components.map((item) => ({
                name: item.name,
                title: item.title,
            })),
            logics: h5Logics.map((item) => ({
                name: item.name,
                description: item.description,
            })),
        });
